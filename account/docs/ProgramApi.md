...
with ebayaccount.ApiClient(configuration) as api_client:
    ...
    api_instance = ebayaccount.ProgramApi(api_client)
...
# **opt_in_to_program**
> object opt_in_to_program(content_type, program)
...
with ebayaccount.ApiClient(configuration) as api_client:
    ...
    program = ebayaccount.Program() # Program | Program being opted-in to.

    try:
        api_response = api_instance.opt_in_to_program(content_type, program)
...
# **opt_out_of_program**
> object opt_out_of_program(content_type, program)
...
with ebayaccount.ApiClient(configuration) as api_client:
    ...
    api_instance = ebayaccount.ProgramApi(api_client)
    content_type = 'content_type_example' # str | This header indicates the format of the request body provided by the client. Its value should be set to <b>application/json</b>. <br><br> For more information, refer to <a href=\"/api-docs/static/rest-request-components.html#HTTP\" target=\"_blank \">HTTP request headers</a>.
    program = ebayaccount.Program() # Program | Program being opted-out of.

    try:
        api_response = api_instance.opt_out_of_program(content_type, program)
