...
class Payout(BaseModel):
    ...
    total_fee_details: Optional[List[Fee]] = Field(default=None, description="This array indicates all payout fees associated with a payout, including the fee type, amount, value, and currency.", alias="totalFeeDetails")
    transaction_count: Optional[StrictInt] = Field(default=None, description="This integer value indicates the number of monetary transactions (all orders, refunds, and credits, etc.) that have occurred with the corresponding payout. Its value should always be at least <code>1</code>, since there is at least one order per seller payout.<br/><br/>For split payouts, each of the two sibling payouts would be considered its own transaction. Because of this, if a seller had a payout for one order, but split the order between two accounts, the value would be <code>2</code> instead of <code>1</code>.<br><br><span class=\"tablenote\"><b>Note: </b>Split-payout functionality is <b>only</b> applicable to mainland China sellers. </span>", alias="transactionCount")
    __properties: ClassVar[List[str]] = ["amount", "bankReference", "lastAttemptedPayoutDate", "payoutDate", "payoutId", "payoutInstrument", "payoutMemo", "payoutReference", "payoutStatus", "payoutStatusDescription", "totalAmount", "totalFee", "totalFeeDetails", "transactionCount"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in total_fee_details (list)
        _items = []
        if self.total_fee_details:
            for _item_total_fee_details in self.total_fee_details:
                if _item_total_fee_details:
                    _items.append(_item_total_fee_details.to_dict())
            _dict['totalFeeDetails'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "totalFeeDetails": [Fee.from_dict(_item) for _item in obj["totalFeeDetails"]] if obj.get("totalFeeDetails") is not None else None,
