...
class PagedEligibleItemCollection(BaseModel):
    ...
    """ # noqa: E501
    eligible_items: Optional[List[EligibleItem]] = Field(default=None, description="A list of items that are eligible for a seller-initiated offer to a buyer.  <br><br>Each element in the list contains the listing ID<!-- <i>or</i> the SKU value--> of a listed item. These IDs represent the listings for which buyers have shown an interest.", alias="eligibleItems")
    ...
    total: Optional[StrictInt] = Field(default=None, description="The total number of items retrieved in the result set.  <br><br>If no items match the search criteria, the server returns the HTTP status code <br><code>204 No Content</code>.")
