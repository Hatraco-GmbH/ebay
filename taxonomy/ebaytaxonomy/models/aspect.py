...
class Aspect(BaseModel):
    ...
    aspect_values: Optional[List[AspectValue]] = Field(default=None, description="A list of valid values for this aspect (for example: <code>Red</code>, <code>Green</code>, and <code>Blue</code>), along with any constraints on those values.", alias="aspectValues")
    localized_aspect_name: Optional[StrictStr] = Field(default=None, description="The localized name of this aspect (for example: <code>Colour</code> on the eBay UK site). <br><br><span class=\"tablenote\"> <strong>Note:</strong> This name is always localized for the specified marketplace. </span>", alias="localizedAspectName")
